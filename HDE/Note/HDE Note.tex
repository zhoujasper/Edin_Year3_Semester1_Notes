...
\textbf{Inner Product}: $(f,g) =\int^b_{a}f(x)g(x)dx$ \ \ \ \ \ \ For funcs $f,g$ in $(a,b)$ \ \ \ \ || \ \ \ \ \textbf{Orthogonal}: $(f,g)=0$
...
\textbf{Extension of a function}: {\scriptsize $f(x)$ know for $x\in[0,L]$ \ \ $\Rightarrow$ \ \ Even Ext.: $h(x)=\{f(x),x\in[0,L]; \ f(-x),x\in(-L,0)$ \ || \ Odd Ext.: $h(x)=\{f(x),x\in(0,L); \ 0,x=\pm L,0; \ f(-x)\in(-L,0)$}

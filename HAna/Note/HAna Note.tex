...
\textbf{Structure of open sets}: If $S$ is a nonempty open set \ $\Leftrightarrow$ \ $S$ = \underline{countable} \textit{union} of \underline{disjoint} \textit{open intervals}. {\tiny ps:即 open set = $\cup$ open intervals}
...
$\cdot$ \textbf{Lebesgue Outer Measure (Version 2)}: $m^*(E)=\inf\left\{\lambda(U),E\subseteq U,U \ is \ open \ set \right\}$ \ \ \ \ \ \ ps:$\lambda(U)=\sum \lambda(I_n)$

$\cdot$ \textbf{Lebesgue Outer Measure (Interval)}: If $J\subseteq\mathbb{R}$ is interval. $m^*(J)=\lambda(J)$
...
\textbf{Property of Lebesgue Outer Measure}: Let Set $E,F \ s.t. \ E\subseteq F\subset\mathbb{R}$. \ \ \ \ \ \ Emptyset: $\emptyset$

$\cdot$ \textbf{1. Null Empty Set}: $m^*(\emptyset)=0$. \ \ \ \ \ \ \textbf{2. Monotonicity}: $m^*(E)\leq m^*(F)$. \ \ \ \ \ \ \textbf{3. Countable Subadditivity}: $m^*(\cup_{n=1}^{\infty}E_n)\leq\sum_{n=1}^{\infty}m^*(E_n)$.

\textbf{Measure Zero}: A set $E$ is said to have \textit{measure zero} if $m^*(E)=0$. \hspace{200pt}  {\footnotesize e.g. Cantor Set. $\Downarrow$}

$\cdot$ Every countable subset of $\mathbb{R}$ has measure zero. \ \ \ || \ \ \ {\footnotesize $E\subseteq\mathbb{R}$ countable, \ $\Rightarrow$ \ $E=\cup^\infty_{j=1}\{x_j\}$ {\scriptsize (Union of countable singletons)}} \ \ \ || \ \ \ {\footnotesize $\exists m^*(E)=0,E \ uncountable$}
...
$\cdot$ \textbf{Lemma}: If $^1E,F$ are nonempty closed sets, $^2E\cap F=\emptyset$, $^3E$ bounded. \ \ $\Rightarrow$ \ \ $dist(E,F)>0$

\textbf{Theorem}: If $dist(E,F)>0$ \ \ $\Rightarrow$ \ \ $m^*(E\cup F)=m^*(E)+m^*(F)$.

\textbf{Theorem |Disjoint Closed Sets}: If $(K_n)_{n\in\mathbb{N}}$ is disjoint, closed, \textit{bounded} sets \ \ $\Rightarrow$ \ \ $m^*(\cup^\infty_{n}K_n)=\sum^\infty_{n=1} m^*(K_n)$

\textbf{Theorem |Disjoint Open Sets}: If $(K_n)_{n\in\mathbb{N}}$ is disjoint, open sets \ \ $\Rightarrow$ \ \ $m^*(\cup^\infty_{n}K_n)=\sum^\infty_{n=1} m^*(K_n)$

\textbf{Theorem |Disjoint Intervals}: If $(I_n)_{n\in\mathbb{N}}$ be disjoint intervals. \ \ $\Rightarrow$ \ \ $m^*(\cup^\infty_{n}I_n)=\sum^\infty_{n=1} m^*(I_n)$
...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    \item \textbf{Closed Set}: Any closed set is Lebesgue measurable. \ \ \ \ \ \ {\footnotesize ps: 由这个可以推导: interval / open set 也是}
    \item \textbf{Measure Zero Set}: Any set of measure zero is Lebesgue measurable.
    \item \textbf{Union}: if $(E_n)_{n\in\mathbb{N}}$ is \textit{Lebesgue Measurable} \ \ $\Rightarrow$ \ \ $E:=\cup^\infty_{n=1}E_n$ is \textit{Lebesgue Measurable}.
...
\textbf{Countable Additivity for Lebesgue Measure $\star$}: If $(E_n)_{n\in\mathbb{N}}$ \textit{disjoint} \& \textit{Lebesuge Measurable}.\ \ $\Rightarrow$ \ \ $m(\cup^\infty_{n=1}E_n)=\sum^\infty_{n=1}m(E_n)$

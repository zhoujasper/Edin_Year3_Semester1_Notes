...
\textbf{Theorem}: If $F$ is closed set, \ \ $\Leftrightarrow$ \ \ $\forall(x_n)_{n\in\mathbb{N}}\in F$ convergent. $x=\lim x_n\in F$
...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    \item \textbf{Lebesgue Outer Measure (Version 2)}: $m^*(E)=\inf\left\{\lambda(U),E\subseteq U,U \ is \ open \ set \right\}$ \ \ \ \ \ \ ps:$\lambda(U)=\sum \lambda(I_n)$
...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    ...
    \item \textbf{decreasing$\downarrow$}: If $E_{n+1}\subseteq E_n$ \ \ $\Rightarrow$ \ \ $m(\cap^\infty_{n=1}E_n)=\lim\limits_{n\to\infty} m(E_n)$ \ \ \ {\footnotesize (Hint: $F_n:=E_n\setminus E,E=\cap E_n$)}
\end{enumerate}

...
\textbf{Intractable Posterior}: {\footnotesize 算不出具体分布或不conjugate的model. For some models, the posterior distribution is intractable.}

\textbf{Monte Carlo Methods}: Any func $g(\underline{\theta})$ || $I_g=\int_{\underline{\theta}\in\underline{\Theta}}g(\underline{\theta})p(\underline{\theta}|\mathbf{y})d\underline{\theta}\approx\frac{1}{M}\sum^M_{i=1}g(\underline{\theta}^{(i)})$ \ \ \ $\underline{\theta}^{(i)}\sim p(\underline{\theta}|\mathbf{y}),i=1,...,M$ \ \ \ {\tiny (通过随机采样来逼近积分结果)}

...
\begin{longtable}{|c||l|l|}
    ...
    \hline
    Other & \textbf{Fitted Values}: $\hat{\mu}_i=x_i\hat{\beta}$ \ \ || \ \ \textbf{Residuals}: $\widehat{\varepsilon_i}=y_i-\widehat{\mu}_i$ & For CI / $H_0/H_1$ Test: Assume $\varepsilon_i\sim\mathcal{N}(0,\sigma^2)$ \\
    \hline
\end{longtable}
}
\vspace{-12pt}
...
\textbf{Linear Model}: $Y_i=\mu_i+\varepsilon_i$ \ , \ $i=1,2,..,n$ \ \ $\mu=f(\beta,\mathbf{x})$ \ \ || \ \ {\footnotesize $f(\beta,\mathbf{x})$ linear.} \ \ || \ \ {\footnotesize $\varepsilon_i\stackrel{i.i.d.}{\sim}$ distribution. \ \ \ $\mathbb{E}[\varepsilon_i]=0$ \ \ \ $Var[\varepsilon_i]=\sigma^2$}
...
$\cdot$ \textbf{Some Defs}: {\footnotesize \textbf{Parameter Estimates}: $\underline{\widehat{\beta}}$ \ \ \ | \ \ \ \textbf{Estimated / Fitted Value Mean Vector}: $\underline{\widehat{\mu}}=\mathbf{X}\underline{\widehat{\beta}}$ \ \ \ | \ \ \ \textbf{Estimated / Fitted Residual} Vector: $\underline{\widehat{\varepsilon}}=\mathbf{\underline{y}}-\underline{\widehat{\mu}}$}
...
\textbf{Prediction | PI}: {\footnotesize Create a \underline{Prediction Matrix}: $\mathbf{X}^*$} {\scriptsize ($\mathbf{X}^*$形式和$\mathbf{X}$保持一直,数据变为按拟合后的每个点的数据)} \ \ $\Rightarrow$ \ \ {\footnotesize $\underline{\widehat{\mu}}^*=\mathbf{X}^*\underline{\widehat{\beta}}$ \ , \ $\underline{\widehat{\mu}}^*\sim\mathcal{N}(\underline{\mu}^*,X^*(X^TX)^{-1}(X^*)^T\sigma^2)$}

...
\begin{longtable}{|c||l|l|l|}
    \hline
    SL Estimate & $S_{xy}=\frac{1}{n}\sum(x_i-\overline{x})(y_i-\overline{y})=\sum x_i^2-\frac{1}{n}(\sum x_i)^2$ & LS Method: $\widehat{\beta}=\frac{S_{xy}}{S_{xx}}$ | $\widehat{\alpha}=\overline{y}-\widehat{\beta}\overline{x}$ & {\scriptsize $\widehat{\alpha},\widehat{\beta}=\arg\min_{\alpha,\beta}S(\alpha,\beta)$ | $S(\alpha,\beta)=\sum(y_i-\mu_i)$} \\
    \hline
    $\mathbb{E},Var$ For $\beta$ & $\mathbb{E}[\widehat{\beta}]=\beta${\tiny ,unbiased} \ \ || \ \ $Var[\widehat{\beta}]=\sigma^2_{\widehat{\beta}}=\frac{\sigma^2}{S_{xx}}$ & $\widehat{Var[\widehat{\beta}]}=\widehat{\sigma^2}_{\widehat{\beta}}=\frac{\widehat{\sigma^2}}{S_{xx}}$ & $\widehat{\sigma^2}=\frac{1}{n-2}\sum(y-\alpha-\beta x)^2$ \\
    \hline
    $\mathbb{E},Var$ For $\alpha$ & $\mathbb{E}[\widehat{\alpha}]=\alpha${\tiny ,unbiased} \ \ || \ \ {\scriptsize $Var[\widehat{\alpha}]=\sigma^2_{\widehat{\alpha}}=\sigma^2\left(\frac{1}{n}+\frac{\overline{x}^2}{S_{xx}}\right)$} & $\widehat{Var[\widehat{\alpha}]}=\widehat{\sigma^2}_{\widehat{\alpha}}=\widehat{\sigma^2}\left(\frac{1}{n}+\frac{\overline{x}^2}{S_{xx}}\right)$ & $\widehat{\sigma^2}=\frac{1}{n-2}\sum(y-\alpha-\beta x)^2$ \\
    \hline
    \hline
    \makecell{$H_0,H_1$ Test || CI} & $T= (\widehat{\beta}- \beta_0) \ / \ Var(\widehat{\beta})\sim\mathcal{N}(0,1)$ \ \ {\tiny ($\sigma^2$ known)} & {\scriptsize $T=(\widehat{\beta}-\beta_0) \ / \ \widehat{Var(\widehat{\beta})}\sim t_{n-2}$} \ \ {\tiny ($\sigma^2$ unknown)} & $T=(\widehat{\alpha}-\alpha_0) \ / \ \widehat{Var(\widehat{\alpha})}\sim t_{n-2}$ \\
    \hline
    (ctd.) Other & {\scriptsize \textbf{Fitted Values}: $\hat{\mu}_i=\hat{\alpha}+\hat{\beta}x_i$ || \textbf{Residuals}: $\widehat{\varepsilon_i}=y_i-\widehat{\mu}_i$} & {\scriptsize For CI / PI / $H_0/H_1$ Assume $\varepsilon_i\sim\mathcal{N}(0,\sigma^2)$} & $T=(\widehat{\alpha}-\alpha_0)/Var(\widehat{\alpha})\sim\mathcal{N}(0,1)$ \\
    \hline

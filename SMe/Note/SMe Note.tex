...
\begin{longtable}{|c|c|c|c|}
    ...
    \hline
    Score Test & $\mathbf{U}(\theta_0)^T\mathbf{\mathcal{I}}^{-1}(\theta_0)\mathbf{U}(\theta_0)>z_{\alpha,p}$ & $\theta=\theta_0$ & $\{\theta:\mathbf{U}(\widehat{\theta})^T\mathbf{\mathcal{I}}^{-1}(\theta)\mathbf{U}(\theta)< z_{\alpha,p}\}$ \\
    \hline
...
% \vspace{-0.25cm}
% \hspace{15pt}{\tiny * In this section, we will not distinguish between r.v.'s and associated realisations. }
% \vspace{-0.15cm}
...
\begin{longtable}{|c|c|c|c|}
    ...
    \hline
    {\scriptsize Single Para} & {\footnotesize $Var[\theta|\mathbf{y}]=\int_{\theta\in\Theta}(\theta-\mathbb{E}[\theta|\mathbf{y}])^2p(\theta|\mathbf{y})d\theta$} & \multicolumn{2}{l|}{\footnotesize $\mathbb{P}(a\leq\theta\leq b)=\int_{a}^{b}p(\theta|\mathbf{y})d\theta=1-\alpha$ \ \ \ \ \ \ ps: $\mathbb{P}(\theta<a|\mathbf{y})=\mathbb{P}(\theta>b|\mathbf{y})=\alpha/2$} \\
    \hline
\end{longtable}
\vspace{-0.35cm}
...
\hspace{70pt} {\footnotesize $^2n$ middle $\Rightarrow$ posterior less affected by choice of prior. \ $^3n$ large $\Rightarrow$ choice of prior has limited effect on posterior.}
...
\begin{longtable}{|l||c|c|c|c|}
    ...
    \hline
    \hline
    {\textbf{Bernoulli} \ $X\sim Bern(p)$} & $p^{k}(1-p)^{1-k},k\in\{0,1\}$ & $p$ & $p(1-p)$ & {\small If $X_i$ are i.i.d. $\sum X_i\sim Bin(n,p)$} \\
    \hline
    {\textbf{Binomial} \ $X\sim Bin(n,p)$} & $\binom{n}{k}p^k(1-p)^{n-k}$ & $np$ & $np(1-p)$ & \makecell{$X,Y\sim Bin(n,p),Bin(m,p)$ \\ $X+Y\sim Bin(n+m,p)$} \\
    \hline
    ...
    \hline
    \makecell{\textbf{Geometric} \ $X\sim Geom(p)$} & $(1-p)^{k-1}p$ & $1/p$ & $(1-p)/p^2$ & $\mathbb{E}[X^2]=2/p^2$ \\
    \hline
    \makecell{\small \textbf{Negative Binomial} $X\sim NB(r,p)$} & $\binom{r+k-1}{k}p^r(1-p)^{k}$ , $r\geq1$ & {\small $\frac{r(1-p)}{p}$} & {\small $\frac{r(1-p)}{p^2}$} &  \\
    \hline
    {\textbf{Poission} \ $X\sim Po(\lambda)$} & $e^{-\lambda}\cdot \frac{\lambda^k}{k!}$ & $\lambda$ & $\lambda$ & $\mathbb{E}[X^2]=\lambda^2+\lambda$ \\
    \hline
...
\textbf{Linear Regression Assumptions}: Assumptions are that the obervations: 1. are from a normal distribution; 2. are independent; 3. have a common variance; 4. Linearity of the expectation variable.

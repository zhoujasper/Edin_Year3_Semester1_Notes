...
\textbf{Prediction | PI}: {\scriptsize Create a \underline{Prediction Matrix}: $\mathbf{X}^*$} {\scriptsize ($\mathbf{X}^*$形式和$\mathbf{X}$保持一直,数据变为按拟合后的每个点的数据)} \ \ $\Rightarrow$ \ \ {\tiny $\sigma^2$ 已知} {\footnotesize $\underline{\widehat{\mu}}^*=\mathbf{X}^*\underline{\widehat{\beta}}$ \ , \ $\underline{\widehat{\mu}}^*\sim\mathcal{N}(\underline{\mu}^*,X^*(X^TX)^{-1}(X^*)^T\sigma^2)$}
...
\begin{enumerate}[itemsep=-3pt, topsep=0pt]
    ...
    {\tiny ps: 点在整张图均匀分布$\to$constant variance}
...
\textbf{Check balance of quantiles}: {\footnotesize 检查残差的分布是否对称.} Method \ 1: $|\frac{\widehat{\underline{\varepsilon}}_{median}}{\widehat{\sigma}}|$ \ \ Method \ 2: $|\frac{\widehat{\underline{\varepsilon}}_{Q3}-\widehat{\underline{\varepsilon}}_{Q1}}{\widehat{\sigma}}|$ {\tiny small $\Rightarrow$ weak evidence against residuals not being symmetric}
...
\textbf{Linear Model}: 
\textbf{LS Estimation}: $S(\beta)$ small: $\mu_i$ relatively close to the $y_i$; \ \ Large: $\mu_i$ far from their corresponding $y_i$. \ \ \ || \ \ \ 
\textbf{Meaning of $\varepsilon$}: The random component of the model is intended to capture the fact that if we gathered a replicate set of data, the $\beta_i$ (coefficient) would not change, but the $Y_i$ would, as a result of different measurement errors. \ \ \ || \ \ \ 
\textbf{Why Use Least Square to estimate $\underline{\beta}$}: Most of time, Linear Model can be a system of $n$ equations with $p$ parameters with $n>p$. In this case, the system is overdetermined and there is no exact solution. The least square method is used to find the best solution. \ \ \ || \ \ \ 
...
\textbf{Meaning of $R^2$}: $R^2$ evaluates how weel the model fits the response data. \ \ \ || \ \ \ 后面减去的部分: proportion of the variance in the response variable that is not explained by the model. \ \ \ || \ \ \ \textbf{$\to$1}: A value close to 1 indicates that the model fits well and can explain most of the variability. \ \ \ || \ \ \ \textbf{$\to$ 0}: $\star$ a low $R^2$ value does \textit{not necessarily} indicate a poor model; it may simply mean that the data contains a substantial random component.

\textbf{For Model of Factors}: If it's a model of Factors, it's often difficult to interpret the p-value for individual coefficients in any meaningful way. $\Rightarrow$ Prefer to test the whole factor effect.

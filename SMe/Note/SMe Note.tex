...
\begin{enumerate}[itemsep=-2pt, topsep=-2pt]
    \item \textbf{Importance Samplimg (IS)}: $^1$ Simulating samples from another distribution $q(\underline{\theta})$ (\textit{Proposal Distribution})
...
\textbf{Laplace Approximation}: {\footnotesize 用高斯分布来近似后验分布. 适用于后验分布是单峰的情况.}
